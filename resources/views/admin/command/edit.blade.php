                ...
                <div class="box">
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title">编辑指令</h3>
                        </div>
                        <form role="form" action="/admin/commands/{{$command->id}}" method="POST">
                            {{ method_field('PUT') }}
                            {{ csrf_field() }}
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="" class="form-control">
                                        @foreach(Config::get('constants.command_types') as $command_type)
                                            <option value="{{ $command_type['value'] }}" @if ($command_type['value']==$command->type) selected @endif>{{ $command_type['name']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
